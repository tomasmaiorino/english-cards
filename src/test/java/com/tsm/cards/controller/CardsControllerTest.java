...
public class CardsControllerTest {
	...
	@SuppressWarnings("unchecked")
	@Test
	public void delete_NotFoundCardResourceGiven_ShouldSaveCard() {
		...
		// Assertions
		verify(mockService).findById(CARD_ID);
		verify(mockService, times(0)).delete(anyObject());
	}

	@Test
	public void delete_ValidCardResourceGiven_ShouldSaveCard() {
		// Set up
		Card origin = CardTestBuilder.buildModel();
		ReflectionTestUtils.setField(origin, "id", CARD_ID);
		...
		// Expectations
		when(mockService.findById(CARD_ID)).thenReturn(origin);
		doNothing().when(mockService).delete(origin);
		...
		// Assertions
		verify(mockService).findById(CARD_ID);
		verify(mockService).delete(origin);
