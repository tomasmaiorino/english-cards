...
public class UsersControllerIT extends BaseTestIT {
	...
	public void save_ValidResourceGiven_ShouldSaveClient() {
		...
		// Do Test
		given().headers(getHeader()).body(resource).contentType(ContentType.JSON).when().post(USER_AUTH_URL).then()
				.statusCode(HttpStatus.OK.value()).body(TOKEN_PARAM, notNullValue());
	}
